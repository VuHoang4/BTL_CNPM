...
class VaiTro(Enum):
    ...
    # def __str__(self):
    #     # Return a user-friendly string representation of the role
    #     if self == VaiTro.GIAOVIEN:
    #         return "Giáo viên"
    #     elif self == VaiTro.GIAOVU:
    #         return "Giáo vụ"
    #     elif self == VaiTro.QUANTRI:
    #         return "Quản trị"
...
class HocKy(db.Model):
    ...
    hocBong = relationship('HocBong', backref='hocKy', lazy=True)
...
# Học bổng
class HocBong(db.Model):
    __tablename__='HocBong'
    maHocBong = Column(String(50), primary_key=True, nullable=False)  # Mã học bổng
    maHocKy = Column(String(10), ForeignKey('HocKy.maHocKy'), nullable=False)
    maHocSinh = Column(String(10), ForeignKey('HocSinh.maHocSinh'), nullable=False)
