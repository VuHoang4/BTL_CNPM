from App.models import NguoiDung, MonHoc, VaiTro, Lop,Khoi, QuyDinh, HocSinh, Diem,HocKy,HocBong, HocSinhLop,LopMonHoc
...
def add_hoc_bong(maHocKy, maHocSinh):
    ma_hoc_bong = generate_ma_hoc_bong(maHocKy)

    existing_hoc_bong = HocBong.query.filter_by(maHocKy=maHocKy, maHocSinh=maHocSinh).first()
    if existing_hoc_bong:
        logging.warning(f"Học bổng đã tồn tại cho học sinh {maHocSinh} trong kỳ học {maHocKy}.")
        return

    new_hoc_bong = HocBong(
        maHocBong=ma_hoc_bong,
        maHocKy=maHocKy,
        maHocSinh=maHocSinh
    )
    add_object_to_db(new_hoc_bong)
...
def generate_random_score():
    return round(random.uniform(1, 10), 1)  # Điểm ngẫu nhiên từ 1 đến 10 với .5
...
def generate_ma_hoc_bong(maHocKy):
    # Tìm số học bổng hiện tại cho học kỳ
    existing_hoc_bongs = HocBong.query.filter(HocBong.maHocBong.like(f'HB{maHocKy}%')).all()

    # Nếu chưa có học bổng nào, bắt đầu từ 1
    if not existing_hoc_bongs:
        next_number = 1
    else:
        # Lấy số lớn nhất đã tồn tại và tăng lên 1
        existing_numbers = [int(hb.maHocBong[-3:]) for hb in existing_hoc_bongs]
        next_number = max(existing_numbers) + 1

    # Đảm bảo số luôn có 3 chữ số
    return f"HB{maHocKy}{next_number:03}"
